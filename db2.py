...
Всякий раз, когда подключается новый пользователь, его данные записываются в словарь user_data, а для сохранения в файл нужно перезаписать файл целиком! 
Сейчас это не вызывает особых проблем и ты можешь спокойно пользоваться JSON-ом. Но что делать разработчикам игры или популярной социальной сети, к которой новые пользователи подключаются каждую секунду? 
...
Язык запросов к таким базам данных называется SQL (англ. Structured Query Language — «структурированный язык запросов»). 
По сути, это отдельный язык программирования. Сейчас мы не будем закапываться в написание запросов, но обязательно вернёмся к ним в будущем. Но вот один из наиболее популярных:
...
Язык SQL позволяет эффективно манипулировать данными. А ещё несомненный плюс баз данных на нём — возможность хранить таблицы на удалённом сервере. 

Это очень удобно при командной работе: все имеют непрерывный доступ к последней версии данных. 
  
Но у SQL есть и свои ограничения: данные имеют жёсткую структуру. Это значит, что нужно заранее хорошо продумать, какие таблицы понадобятся, 
какие в них будут поля (столбцы) и как эти таблицы будут связаны между собой. Любые изменения в структуре потребуют изменения всей схемы данных, что очень затратно по времени. 
...
Плюс в том, что в одних документах можно легко добавлять новые поля, не добавляя эти поля в другие. Например, мы легко можем добавить возраст в один из доков:
