...
Вчера мы узнали, что логические выражения могут принимать одно из двух значений: True или False.  Эти значения можно записывать в переменные:

is_cold = True 

Более того, в переменные можно вписывать и сами логические выражения (например, сравнения). А Python вычислит и подставит значение True или False в переменную:

temperature = 8
is_cold = temperature < 10  # is_cold будет равно True, т. к. температура меньше 10 

Здесь мы создали переменную is_cold с результатом сравнения temperature < 10. В итоге получается, что is_cold равно True, потому что температура меньше 10.
